...
// const feedQuery = `
// SELECT u.*, COALESCE(c.comment_count, 0) AS comment_count, COALESCE(l.like_count, 0) AS like_count
// FROM userfeed u
// LEFT JOIN (
//     SELECT feedname, COUNT(*) AS comment_count
//     FROM commentinfo
//     GROUP BY feedname
// ) c ON u.feedname = c.feedname
// LEFT JOIN (
//     SELECT feedname, COUNT(*) AS like_count
//     FROM likeinfo
//     GROUP BY feedname
// ) l ON u.feedname = l.feedname
// ORDER BY (COALESCE(c.comment_count, 0) + COALESCE(l.like_count, 0)) DESC;
// `;
// const feedQuery = `
// SELECT u.*, COALESCE(c.comment_count, 0) AS comment_count, COALESCE(l.like_count, 0) AS like_count
// FROM userfeed u
